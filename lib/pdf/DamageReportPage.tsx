    ...
    <Page size="A4" style={s.page}>
      ...
      <View style={[s.outerBorder, { marginTop: 4 }]}>
        <View style={s.row}>
          <View style={{ flex: 1, borderRight: BORDER }}>
            {sameAsOwner ? (
              <>
                <SectionHeader title="TULAJDONOS / VEZETŐ (AZONOS SZEMÉLY)" />
                <View style={s.row}>
                  <Cell label="Tulajdonos (üzembentartó) és vezető neve" value={data.ownerName} flex={1} noBorderRight />
                </View>
                <View style={s.row}>
                  <Cell label="Cím" value={data.ownerAddress} flex={1} noBorderRight />
                </View>
                <View style={s.row}>
                  <Cell label="Telefonszám" value={data.customerPhone || data.driverPhone} flex={1} />
                  <Cell label="Születési idő" value={data.driverBirthDate ? formatDateOnly(data.driverBirthDate) : undefined} flex={1} noBorderRight />
                </View>
              </>
            ) : (
              <>
                ...
                <View style={s.row}>
                  <Cell label="Vezető neve" value={data.driverName} flex={1} noBorderRight />
                </View>
                <View style={s.row}>
                  <Cell label="Vezető címe" value={data.driverAddress} flex={1} />
                  <Cell label="Telefonszám" value={data.driverPhone} width={95} noBorderRight />
                </View>
                <View style={s.row}>
                  <Cell label="Vezető születési ideje" value={data.driverBirthDate ? formatDateOnly(data.driverBirthDate) : undefined} flex={1} noBorderRight />
                </View>
              </>
            )}
            {hasLicenseData && (
              <View style={s.row}>
                ...
                <Cell
                  ...
                  value={data.driverLicenseValidUntil ? formatDateOnly(data.driverLicenseValidUntil) : undefined}
                  flex={1}
                  noBorderRight
                />
              </View>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginBottom: 4 }}>
                {DAMAGE_TYPE_OPTIONS.map((opt) => (
                  <View key={opt.value} style={{ flexDirection: "row", alignItems: "center" }}>
                    <CheckMark checked={data.damageType === opt.value} />
                    <Text style={{ fontSize: 7, color: "#111827" }}>{opt.label}</Text>
                  </View>
                ))}
              </View>
      ...
      <View wrap={false}>
        <View style={s.outerBorder}>
          ...
          <View style={s.row}>
            ...
            <View style={{ flex: 2 }}>
              ...
              <View style={s.row}>
                <CheckCell label="Fotómásolási engedély" checked={data.consentToPhotocopy} flex={1} />
        ...
        <View style={s.outerBorder}>
          ...
          <View style={s.row}>
            {/* Mindig két doboz. Azonos személy esetén egyszer írnak alá, és ugyanaz az
                aláírás kerül mindkét dobozba. */}
            <View style={[s.cell, { flex: 1, borderRight: BORDER }]}>
              <SignatureBlock
                label="Vezető aláírása"
                signatureDataUrl={sameAsOwner ? data.ownerSignatureUrl : data.driverSignatureUrl}
              />
            </View>
